...
__export(index_exports, {
  ...
  RedemptionModule: () => RedemptionModule,
  SdkError: () => SdkError,
...
async function buildAndSendV0(client, signer, ixs, opts = {}) {
  ...
  const preIxs = [];
  if (opts.computeUnitLimit) preIxs.push(import_web33.ComputeBudgetProgram.setComputeUnitLimit({ units: opts.computeUnitLimit }));
  if (opts.computeUnitPriceMicroLamports) preIxs.push(import_web33.ComputeBudgetProgram.setComputeUnitPrice({ microLamports: opts.computeUnitPriceMicroLamports }));
...
function deriveGlobalConfigPda(programId) {
  return import_web34.PublicKey.findProgramAddressSync([Buffer.from("global_config")], programId);
}
...
var DEFAULT_COLLATERAL_MINT = new import_web35.PublicKey("EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v");
var CreateMarketSchema = import_zod.z.object({
  baseMint: import_zod.z.instanceof(import_web35.PublicKey).optional(),
  quoteMint: import_zod.z.instanceof(import_web35.PublicKey).optional(),
...
var MarketModule = class {
  ...
  async createMarketDerived(params) {
    ...
    const [market] = import_web35.PublicKey.findProgramAddressSync(
      [Buffer.from("market-v2"), gidLe],
      this.client.programId
    );
    const [marketExtension] = import_web35.PublicKey.findProgramAddressSync(
    ...
    const accounts = {
      ...
      system_program: import_web35.SystemProgram.programId
    };
  ...
  async setMarketResolvable(params) {
    ...
    const collateralTokenMint = new import_web35.PublicKey(marketAccount.collateral_token || marketAccount.collateralToken);
    const creator = new import_web35.PublicKey(marketAccount.creator);
    ...
    const TOKEN_METADATA_PROGRAM_ID = new import_web35.PublicKey("metaqbxxUerdq28cj1RbAWkYQm3ybzjb6a8bt518x1s");
    ...
    const accounts = {
      ...
      associated_token_program: import_spl_token2.ASSOCIATED_TOKEN_PROGRAM_ID,
      system_program: import_web35.SystemProgram.programId,
      rent: new import_web35.PublicKey("SysvarRent111111111111111111111111111111111")
    };
...
function buildTradeIx(params) {
  ...
  return new import_web36.TransactionInstruction({ keys, programId: PNP_PROGRAM_ID, data });
}

// src/modules/trading.ts
var import_spl_token3 = require("@solana/spl-token");
var TradeSchema = import_zod2.z.object({
  market: import_zod2.z.instanceof(import_web37.PublicKey),
...
var TradingModule = class _TradingModule {
  ...
  async mintDecisionTokensDerived(params) {
    ...
    const yesTokenMint = new import_web37.PublicKey(m.yesTokenMint ?? m.yes_token_mint);
    const noTokenMint = new import_web37.PublicKey(m.noTokenMint ?? m.no_token_mint);
    const collateralMint = new import_web37.PublicKey(m.collateralToken ?? m.collateral_token);
    const marketCreator = new import_web37.PublicKey(m.creator ?? m.market_creator ?? m.creator);
    const creatorFeeTreasury = new import_web37.PublicKey(m.creatorFeeTreasury ?? m.creator_fee_treasury);
    try {
    ...
    const admin = new import_web37.PublicKey(g.admin);
    ...
    const accounts = {
      ...
      system_program: import_web37.SystemProgram.programId
    };
  ...
  async getMarketInfo(market) {
    ...
    return {
      ...
      question: m.question,
      id: m.id ?? m.market_id,
      creator: new import_web37.PublicKey(m.creator ?? m.market_creator ?? m.creator),
      ...
      yesTokenMint: new import_web37.PublicKey(m.yesTokenMint ?? m.yes_token_mint),
      noTokenMint: new import_web37.PublicKey(m.noTokenMint ?? m.no_token_mint),
      collateralToken: new import_web37.PublicKey(m.collateralToken ?? m.collateral_token),
  ...
  async buyTokensUsdc(params) {
    ...
    try {
      ...
      if (e instanceof import_web37.SendTransactionError) {
...
function buildRedeemIx(params) {
  ...
  return new import_web38.TransactionInstruction({ keys, programId: PNP_PROGRAM_ID, data });
}

// src/modules/redemption.ts
var RedeemSchema = import_zod3.z.object({
  market: import_zod3.z.instanceof(import_web39.PublicKey),
  position: import_zod3.z.instanceof(import_web39.PublicKey),
  amount: import_zod3.z.bigint(),
  to: import_zod3.z.instanceof(import_web39.PublicKey).optional()
});
...
var NodeKeypairSigner = class {
  ...
  async signTransaction(tx) {
    if (tx instanceof import_web310.VersionedTransaction) {
