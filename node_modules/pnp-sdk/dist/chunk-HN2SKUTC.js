...
var ProgramError = class extends SdkError {
  programErrorCode;
  logs;
  constructor(message, details, programErrorCode, logs) {
    super("PROGRAM_ERROR", message, details);
...
function deriveMarketPda(baseMint, quoteMint, programId) {
...
function derivePositionPda(market, owner, programId) {
  return PublicKey3.findProgramAddressSync([
...
function deriveMarketPdaFromMints(yesMint, noMint, programId) {
  return PublicKey3.findProgramAddressSync([
    Buffer.from("market"),
    yesMint.toBuffer(),
    noMint.toBuffer()
...
import { PublicKey as PublicKey5, SystemProgram as SystemProgram2 } from "@solana/web3.js";
import { z } from "zod";
...
function buildCreateMarketIxIdl(accounts, args, programId) {
  ...
  const initial_liquidity = new BN(args.initial_liquidity.toString());
  const end_time = new BN(args.end_time.toString());
  if (DEBUG2) {
...
async function buildAndSendV0(client, signer, ixs, opts = {}) {
  ...
  const preIxs = [];
  if (opts.computeUnitLimit) preIxs.push(ComputeBudgetProgram.setComputeUnitLimit({ units: opts.computeUnitLimit }));
  if (opts.computeUnitPriceMicroLamports) preIxs.push(ComputeBudgetProgram.setComputeUnitPrice({ microLamports: opts.computeUnitPriceMicroLamports }));
...
var CreateMarketSchema = z.object({
  baseMint: z.instanceof(PublicKey5).optional(),
  quoteMint: z.instanceof(PublicKey5).optional(),
  question: z.string().optional(),
  initialLiquidity: z.bigint().optional(),
  endTime: z.bigint().optional(),
  tickSize: z.bigint().optional(),
  lotSize: z.bigint().optional()
});
var MarketModule = class {
  ...
  async createMarketDerived(params) {
    ...
    const [market] = PublicKey5.findProgramAddressSync(
...
import { PublicKey as PublicKey7, SystemProgram as SystemProgram3, SendTransactionError } from "@solana/web3.js";
...
function buildTradeIx(params) {
  ...
  return new TransactionInstruction3({ keys, programId: PNP_PROGRAM_ID, data });
}
...
var TradingModule = class _TradingModule {
  ...
  async mintDecisionTokensDerived(params) {
    ...
    const yesTokenMint = new PublicKey7(m.yesTokenMint ?? m.yes_token_mint);
    const noTokenMint = new PublicKey7(m.noTokenMint ?? m.no_token_mint);
    const collateralMint = new PublicKey7(m.collateralToken ?? m.collateral_token);
    const marketCreator = new PublicKey7(m.creator ?? m.market_creator ?? m.creator);
    const creatorFeeTreasury = new PublicKey7(m.creatorFeeTreasury ?? m.creator_fee_treasury);
    try {
    ...
    const admin = new PublicKey7(g.admin);
  ...
  async getMarketInfo(market) {
    ...
    return {
      ...
      question: m.question,
      id: m.id ?? m.market_id,
      creator: new PublicKey7(m.creator ?? m.market_creator ?? m.creator),
      ...
      yesTokenMint: new PublicKey7(m.yesTokenMint ?? m.yes_token_mint),
      noTokenMint: new PublicKey7(m.noTokenMint ?? m.no_token_mint),
      collateralToken: new PublicKey7(m.collateralToken ?? m.collateral_token),
  ...
  async sellTokensBase(params) {
    ...
    try {
      ...
      const mintInfo = await getMint(this.client.connection, mint, this.client.commitment).catch(() => void 0);
      const decimals = mintInfo?.decimals ?? 6;
  ...
  async redeemPosition(market) {
    ...
    const accounts = {
      ...
      admin_collateral_token_account: adminCollateralTokenAccount,
      creator_fee_treasury: creatorFeeTreasury,
      token_program: TOKEN_PROGRAM_ID3,
      associated_token_program: ASSOCIATED_TOKEN_PROGRAM_ID3,
      system_program: SystemProgram3.programId
    };
...
function buildRedeemIx(params) {
  ...
  return new TransactionInstruction5({ keys, programId: PNP_PROGRAM_ID, data });
}

// src/modules/redemption.ts
var RedeemSchema = z3.object({
  market: z3.instanceof(PublicKey9),
  position: z3.instanceof(PublicKey9),
  amount: z3.bigint(),
  to: z3.instanceof(PublicKey9).optional()
});
...
var AnchorMarketModule = class {
  ...
  async createMarket(params) {
    ...
    if (params.yesOddsBps !== void 0 || params.oracle !== void 0) {
      [marketExtension] = PublicKey12.findProgramAddressSync(
    ...
    const payerCollateralTokenAccount = await getAssociatedTokenAddress(
      collateralTokenMint,
      this.anchorClient.walletPublicKey
    );
    const marketReserveVault = await getAssociatedTokenAddress(
      collateralTokenMint,
      market,
      true
      // allowOwnerOffCurve
    ...
    try {
      const computeBudgetIx = ComputeBudgetProgram2.setComputeUnitLimit({
        units: 4e5
      });
      const txSignature = await this.anchorClient.program.methods.createMarketV2(
        ...
        associatedTokenProgram: ASSOCIATED_TOKEN_PROGRAM_ID4,
...
var AnchorMarketV3Module = class {
  ...
  async createMarketV3(params) {
    ...
    if (params.oddsBps !== void 0 || params.oracle !== void 0) {
      [marketExtension] = PublicKey13.findProgramAddressSync(
    ...
    const payerCollateralTokenAccount = await getAssociatedTokenAddress2(
      collateralTokenMint,
      this.anchorClient.walletPublicKey
    );
    const marketReserveVault = await getAssociatedTokenAddress2(
      collateralTokenMint,
      market,
      true
      // allowOwnerOffCurve
    );
    const creatorYesTokenAccount = await getAssociatedTokenAddress2(
      yesTokenMint,
      creatorOverride
    );
    const creatorNoTokenAccount = await getAssociatedTokenAddress2(
      noTokenMint,
      creatorOverride
    );
    ...
    try {
      const computeBudgetIx = ComputeBudgetProgram3.setComputeUnitLimit({
        units: 4e5
      });
      const txSignature = await this.anchorClient.program.methods.createMarketV3(
        ...
        associatedTokenProgram: ASSOCIATED_TOKEN_PROGRAM_ID5,
...
var PNPClient = class _PNPClient {
  ...
  constructor(rpcUrl, privateKey) {
    ...
    if (privateKey) {
      ...
      if (typeof privateKey === "string") {
        try {
          secretKey = Uint8Array.from(bs58.decode(privateKey));
        } catch {
          throw new Error(
  ...
  static parseSecretKey(keyString) {
    ...
    try {
      return Uint8Array.from(bs58.decode(keyString));
    } catch {
    }
  ...
  async buyV3TokensUsdc(params) {
    ...
    try {
      ...
      const buyer = this.signer.publicKey;
      const buyerYesTokenAccount = await getAssociatedTokenAddressSync3(
        yesTokenMint,
        buyer,
        false
      );
      const buyerNoTokenAccount = await getAssociatedTokenAddressSync3(
        noTokenMint,
        buyer,
        false
      );
      const buyerCollateralTokenAccount = await getAssociatedTokenAddressSync3(
        collateralTokenMint,
        buyer,
        false
      );
      const marketReserveVault = await getAssociatedTokenAddressSync3(
        collateralTokenMint,
        market,
        true
        // allowOwnerOffCurve
      );
      const amountBN = new BN3(amount.toString());
      const minimumOut = new BN3(0);
      const result = await (this.anchorClient?.program).methods.buyV3Tokens(
        ...
        associatedTokenProgram: ASSOCIATED_TOKEN_PROGRAM_ID6,
...
Cause: ${err.message}`);
  ...
  async redeemPosition(market, options) {
    ...
    const buyerNoTokenAccount = getAssociatedTokenAddressSync3(
      noTokenMint,
      buyer,
      false,
      noTokenProgram
    );
    const buyerCollateralTokenAccount = getAssociatedTokenAddressSync3(
      collateralTokenMint,
      buyer,
      false,
    ...
    const adminCollateralTokenAccount = getAssociatedTokenAddressSync3(
      collateralTokenMint,
      admin,
      false,
      collateralTokenProgram
  ...
  async redeemV3Position(market) {
    ...
    const redeemerNoTokenAccount = getAssociatedTokenAddressSync3(
      noTokenMint,
      redeemer,
      false,
      TOKEN_PROGRAM_ID6
    );
    const redeemerCollateralTokenAccount = getAssociatedTokenAddressSync3(
      collateralTokenMint,
      redeemer,
      false,
      TOKEN_PROGRAM_ID6
  ...
  async fetchMarkets() {
    ...
    const filters = [
      { memcmp: { offset: 0, bytes: bs58.encode(Buffer.from(discriminator)) } }
    ];
  ...
  async claimMarketRefund(market) {
    ...
    const creatorCollateralTokenAccount = getAssociatedTokenAddressSync3(
      collateralMint,
      creator,
      false,
      collateralTokenProgram
    ...
    try {
      const res = await this.redemption.creatorRefundV2({
        ...
        associated_token_program: ASSOCIATED_TOKEN_PROGRAM_ID6,
        system_program: SystemProgram6.programId
      });
  ...
  async settleMarket(params) {
    ...
    const [yesTokenMint] = PublicKey14.findProgramAddressSync(
      [Buffer.from("yes_token"), gidLe],
      this.anchorClient.program.programId
    );
    const [noTokenMint] = PublicKey14.findProgramAddressSync(
